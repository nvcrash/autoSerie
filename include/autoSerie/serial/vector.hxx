...
struct vector_internal
{
  ...
  {
    return (!is_data_type<Contained>() && !herit_ContiguouslySerializable<Contained>() && !std::is_same<Contained,bool>::value);
  }
  ...
  {
    return (is_data_type<Contained>() && !std::is_same<Contained,bool>::value);
  }

  template <class Contained, class Allocator, class OutputStream,
	    typename std::enable_if<bool_handler<Contained>(), int>::type = 0>
  static inline
  void iwrite_vector(OutputStream& stream, const std::vector<Contained, Allocator>& v)
  {
...
#pragma message( "serialization of a std::vector<bool,t> is depreciated, it allocate and copy data" )
  ...
  template <class Contained, class Allocator, class InputStream,
	    typename std::enable_if<bool_handler<Contained>(), int>::type = 0>
  static inline
  void iread_vector(InputStream& stream, std::vector<Contained, Allocator>& v)
  {
...
#pragma message( "deserialization of a std::vector<bool,t> is depreciated, it allocate and copy data" )
  ...
  static inline
  void iwrite_vector(OutputStream& stream, std::vector<Contained, Allocator>& v)
  {
...
{
  vector_internal::iwrite_vector(stream, v);
}
...
{
  vector_internal::iread_vector(stream, v);
}
